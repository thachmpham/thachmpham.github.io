...
- Each thread has its own stack memory, not shared with others.
- Stores arguments, local variables.

:::
::: {.column width=50%}
 
- On each function call, the stack grows downward from high address to low address.
- Shrinks on function returns.
...
- push instruction put a value onto the stack.  
- pop instruction remove a value from the stack.
- rsp automatically moves to the top of the stack.
...
- call instruction pushes the address of the next instruction (rip) onto the stack then jump to the target function. This address is also called the return address.  
- ret instruction pops the return address from the stack and jump to this address.
...
- call instruction pushes the parent rip onto the stack. 
